...
$(document).ready(function() {
  ...
  <div class="section__container">
    ...
    <c:if test="${sessionScope.memId ne null}">
    	<button class="story__button" onclick="goPage('board/boardForm')">프로젝트 생성</button>
   	</c:if>
 <!--    <button class="story__button">더 많은 프로젝트 보기</button> -->
   
        <!-- 검색폼 시작 -->
	  <form id = 'searchForm' class="searchForm" action="${contextPath}/projectList" method="get">
	   <select name='type'>
	...
	<!-- 페이징 끝 -->
		...
		<form id="actionForm" action="${contextPath}/projectList" method="get">
		  <input type="hidden" name="pageNum" value='<c:out value="${pageMaker.cri.pageNum}"/>'>
		  <input type="hidden" name="amount" value='<c:out value="${pageMaker.cri.amount}"/>'>
